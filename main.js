...
function afficherModaleFinDePartie() {
    ...
    document.querySelector('#scoreModal').textContent = 'Votre score : ' + scoreActuel; // Remplacer "score" par la variable de score actuelle
    } else{
...
function generationIcone() {
    fetch('icones.json')
        ...
        .then(data => {
            ...
            document.querySelector('h1').innerHTML = `Quel est le nom de cette technique en<span class="${classe}">&nbsp;<img src="CLASS/${classe}.png">&nbsp;${classe}</span>&nbsp?`;
            document.querySelector('.icons').innerHTML = `<img src="SPELLS/${image}">`;
            }
        });
}
...
function levenshteinDistance(a, b) {
    ...
    reponse.value.toLowerCase().normalize("NFD").replace(/\p{Diacritic}/gu, "").replace(/\s/g, '').replace(/-/g, '').replace(/:/g, '');
    const c = a.length + 1;
    const d = b.length + 1;
    const r = Array(c);
...
function Differences() {
    let normalizedReponseValue = reponse.value.toLowerCase().normalize("NFD").replace(/\p{Diacritic}/gu, "").replace(/\s/g, '').replace(/-/g, '').replace(/:/g, '');
